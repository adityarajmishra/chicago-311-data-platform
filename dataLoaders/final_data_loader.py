#!/usr/bin/env python3
"""
Final Data Loader for Chicago 311 Data Platform
All database issues resolved with correct API schema mapping
"""
...
class FinalChicago311DataLoader:
    """Final Chicago 311 data loader with correct schema mapping."""
    ...
    def setup_databases(self):
        """Setup all database connections and create tables with correct schema."""
        ...
        try:
            ...
            # PostgreSQL with correct schema including all API fields
            logger.info("Setting up PostgreSQL...")
            class FinalPostgreSQLHandler:
                def __init__(self):
                    self.pg_handler = LocalPostgreSQLHandler()
                    self.create_table()
                
                def create_table(self):
                    # Includes all fields from actual API response
                    create_sql = """
                    ...
                    CREATE TABLE chicago_311_requests (
                        ...
                        computed_region_rpca_8um6 VARCHAR(50),
                        computed_region_vrxf_vc4k VARCHAR(50),
                        computed_region_6mkv_f3dw VARCHAR(50),
                        computed_region_bdys_3d7i VARCHAR(50),
                        computed_region_43wa_7qmu VARCHAR(50),
                        computed_region_du4m_ji7t VARCHAR(50)
                    );
                    """
                ...
                def bulk_insert(self, data):
                    ...
                    for record in data:
                        ...
                        field_mapping = {
                            ...
                            'created_hour': record.get('created_hour'),
                            'created_day_of_week': record.get('created_day_of_week'),
                            'created_month': record.get('created_month'),
                            'x_coordinate': record.get('x_coordinate'),
                            'y_coordinate': record.get('y_coordinate'),
                            'latitude': record.get('latitude'),
                            'longitude': record.get('longitude'),
                            'location_point': record.get('location_point'),
                            'computed_region_rpca_8um6': record.get(':@computed_region_rpca_8um6'),
                            'computed_region_vrxf_vc4k': record.get(':@computed_region_vrxf_vc4k'),
                            'computed_region_6mkv_f3dw': record.get(':@computed_region_6mkv_f3dw'),
                            'computed_region_bdys_3d7i': record.get(':@computed_region_bdys_3d7i'),
                            'computed_region_43wa_7qmu': record.get(':@computed_region_43wa_7qmu'),
                            'computed_region_du4m_ji7t': record.get(':@computed_region_du4m_ji7t')
                        }
                        
                        # Clean values
                        for key, value in field_mapping.items():
                            if isinstance(value, str):
                                clean_record[key] = value[:300] if len(value) > 300 else value
                            else:
                    ...
                    try:
                        ...
                        self.pg_handler.connection.rollback()
                        logger.error(f"PostgreSQL insert error: {e}")
                        return 0
            
            pg_handler = FinalPostgreSQLHandler()
            self.handlers['PostgreSQL'] = pg_handler
            logger.info("✅ PostgreSQL ready")
            
            # DuckDB with matching schema
            logger.info("Setting up DuckDB...")
            class FinalDuckDBHandler:
                def __init__(self):
                    self.duckdb_handler = DuckDBHandler()
                    self.create_table()
                
                def create_table(self):
                    # Create table with all API fields
                    create_sql = """
                    DROP TABLE IF EXISTS chicago_311_requests;
                    CREATE TABLE chicago_311_requests (
                    ...
                    self.duckdb_handler.connection.execute(create_sql)
                    logger.info("✅ DuckDB table created with correct schema")
                ...
                def bulk_insert(self, data):
                    ...
                    try:
                        ...
                        for record in data:
                            ...
                            # Map fields (same as PostgreSQL but without id)
                            field_mapping = {
                                'sr_number': record.get('sr_number'),
                                'sr_type': record.get('sr_type'),
                                'sr_short_code': record.get('sr_short_code'),
                            ...
                            clean_data.append(field_mapping)
                        ...
                        self.duckdb_handler.connection.execute("INSERT INTO chicago_311_requests SELECT * FROM df")
            ...
            duckdb_handler = FinalDuckDBHandler()
            ...
            if 'location' in transformed:
                ...
                if isinstance(location, dict):
                    if 'coordinates' in location:
                        ...
                        transformed['latitude'] = float(location['latitude'])
                        transformed['longitude'] = float(location['longitude'])
    ...
    def insert_to_database(self, db_name: str, data: List[Dict]) -> int:
        ...
        try:
            ...
            if db_name == 'MongoDB':
                ...
                # Elasticsearch bulk insert with NumPy 2.0 compatibility
    ...
    def run_loading(self, total_records: int = 12400000):
        ...
        logger.info(f"🚀 Starting final data loading for {total_records:,} records...")
        ...
        logger.info("=" * 80)
        logger.info("🎉 FINAL DATA LOADING COMPLETE!")
...
def main():
    ...
    try:
        loader = FinalChicago311DataLoader()
        results = loader.run_loading()
        
        # Mark task as completed and proceed to benchmarking
